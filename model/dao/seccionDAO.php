...
class SeccionDAO {
    ...
    public function ExecuteCompleteCombobox($objSeccion) {
        ...
        $this->task->setWhereFields('s0.idCarrera;s0.idTurno;s0.scnIndicador');
        $this->task->setWhereLogical('=;=;=');
        $this->task->setWhereValues($idCarrera . ';' . $idTurno . ';1');
    ...
    public function ExecuteSave($objSeccion) {
        ...
        $this->task->setFields('idSeccion;scnDescripcion;scnCantMaxima;scnInicio;scnFin;scnAnioCreacion;idTurno;idCarrera;scnIndicador');
        $this->task->setValues($descripcon . ';' . $cantMaxima . ';' . $inicio . ';' . $fin . ';' . $anioCreacion . ';' . $idTurno . ';' . $idCarrera . ';' . $indicador);
    ...
    public function ExecuteUpdate($objSeccion) {
        ...
        $this->task->setTables(self::TABLE);
        $this->task->setFields('scnDescripcion;scnCantMaxima;scnInicio;scnFin;idTurno;idCarrera');
        $this->task->setValues($descripcon . ';' . $cantMaxima . ';' . $inicio . ';' . $fin . ';' . $idTurno . ';' . $idCarrera);
